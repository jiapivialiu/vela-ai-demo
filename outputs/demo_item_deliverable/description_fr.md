# Sèche-cheveux — Électroménager de soins personnels

**Catégorie**: Électroménager de soins personnels

Ébauche construite à partir de la vision et de l'OCR : les modèles n'ont pas renvoyé de JSON exploitable.
À réviser avant publication.

**Type de produit :** Sèche-cheveux
**Catégorie :** Électroménager de soins personnels
**Marque / série :** Kangfu

**Points saillants :**
- 8881

**Taille / specs :**
- 1900W
- Model 8881
- 2 styling nozzles included

**Texte sur l'image (latin/chiffres seulement, chinois retiré) :**
- 8881
- KANGFU
- 1900
- Watts
- 1900W
- FU

*(Confiance vision : medium_ocr_fallback)*

## Paramètres

- **Brand**: Kangfu
- **Model**: 8881
- **Product Type**: Sèche-cheveux
- **Power**: 1900Watts
- **Colour**: Non précisé
- **Material**: Non précisé
- **Dimensions**: Non précisé
- **Included Accessories**: 2 buses de concentration
- **Certifications Visible**: Non précisé
- **Country Of Origin**: Non précisé
- **Warranty**: Non précisé
- **Weight**: Non précisé

## Autres attributs
- revision_requise: true
- source_brouillon: heuristique_fr
