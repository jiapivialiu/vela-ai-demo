...
仓库内提供 **Streamlit 试用页**：上传商品主图；**MTWI 文本框标注可勾选提供**（关闭则仅主图跑通链路，无坐标蒙版）。一键预览、下载结果（支持 Mock 离线试流程）。**营销扩展图**在主链路中默认不生成；需要时可侧栏调高数量，或跑 **`src/run_marketing_extras_step.py`**（见 [src/README.md](src/README.md)）。链路中 **4b 文案质检** 与 **4c 加英/加法语法质检** 默认始终执行（真实模式会多次调用 GMI；侧栏「回退与审稿模型」可改模型 ID，见 [CONFIGURATION.md](CONFIGURATION.md)）。
...
- 标注（可选）：保持 **「提供 MTWI 文本框标注」** 勾选时，打开 **[data/demo_one/txt_train/demo_item.txt](data/demo_one/txt_train/demo_item.txt)**，将其中一行（`X1,Y1,…,X4,Y4,文本`）复制到 **「2. 文本框标注」** 文本框，或 **上传该文件**。取消勾选则无需标注（整图去字依赖模型提示词，无四边形蒙版）。
